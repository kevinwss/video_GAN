...
#------------------------------------------
    ...
    def predict_images(self,image_z):
        ...
        sess = tf.Session()
        sess.run(init)
        image_z = image_z.eval(session=sess) #(101,256)
        
        #--------------------------------------------
        #image_z = tf.transpose(image_z, perm=[1, 0])
        print('image_z',image_z.shape)   # (100,256) time*latent vector length
        #image_z = np.reshape(image_z,(1,dim)) (101,256)
        #print('image_z2',image_z.shape)
        attrs = np.array([[1]])

        with self.sess.as_default():
            images = self.sess.run(       #predict
                (self.x_f),
                feed_dict={
        ...
        for i in range(100):
            ...
            figure = images[i,:,:,:]
            figure = Image.fromarray(((figure * 0.5 + 0.5) * 255.0).astype(np.uint8))
            filename = 'video/'+ str(i) + '.png'
