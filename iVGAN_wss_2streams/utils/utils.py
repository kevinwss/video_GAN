...
#--------save image------------
def saveImgBatch(batch, directory, name=''):
    filename = os.path.join(directory, "%s.jpeg" % (name))
    with open(filename, 'wb') as f:
        f.write(batch[0])  # only one frame in batch
...
def convert_image(images, batch_size, col=5, row=5):
    ...
    rows = []
    for i in range(row):
        #for image in images[col * i + 0:col * i + col]:
            #print(image.shape,i)
        #new =tf.concat(images[col * i + 0:col * i + col], 2)
        #print(new.shape,i)
        #if len(images[col * i + 0:col * i + col]) == 0:
        #    continue
        #new =tf.concat(images[col * i + 0:col * i + col], 2)
        #print(new.shape)			
        rows.append(tf.concat(images[col * i + 0:col * i + col], 2)) # error here
...
#--------sample batch for image---------------------

def sample_imageBatch(samples, batch_size, col=5, row=5):
    frames = [convert_image(samples[:, :, :, :], batch_size, col, row)]
    return frames
#----------------------------------------------------
